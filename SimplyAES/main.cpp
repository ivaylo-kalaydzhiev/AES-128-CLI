...
public:
    ...
    static void decrypt(unsigned char *cipherText, unsigned char *key) {
        unsigned char state[BLOCK_SIZE];
        for (int i = 0; i < BLOCK_SIZE; ++i) {
...
int main() {
    ...
    for (int i = 0; i < lenOfPaddedMessage; i += 16) {
        AESCryptographer::encrypt(paddedMessage + i, key);
    }
    ...
    for (int i = 0; i < lenOfPaddedMessage; i += 16) {
        AESCryptographer::decrypt(paddedMessage + i, key);
    }
